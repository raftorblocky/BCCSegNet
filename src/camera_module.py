...
def set_camera_mode(now: datetime) -> str:
    ...
    if(hour > 5 or (hour == 5 and minute >= 40)) and (hour < 17 or (hour == 17 and minute < 40)):
        controls = {'AnalogueGain': 1.0, 'ExposureTime': 100}
        mode = 'day'
    else:
        controls = {'AnalogueGain': 2.0, 'ExposureTime': 2000000}
        mode = 'night'
